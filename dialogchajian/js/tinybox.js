	...
	return {
		show: function (c, u, w, h, a, t) {
			if (!f) {
				...
				b.id = 'tinycontent';
				document.body.appendChild(m); //插入tinymask
				document.body.appendChild(p); //插入tinybox
				p.appendChild(b); //tinybox里插入tinycontent
				m.onclick = TINY.box.hide; //点击tinymask把整个弹窗隐藏
				window.onresize = TINY.box.resize; //浏览器监听窗口变化
				f = 1; //这个标志是啥？
			}
			if (!a && !u) {
				//a u同时为0的时候， tinybox高宽度由w, h来决定
				p.style.width = w ? w + 'px' : 'auto';
				p.style.height = h ? h + 'px' : 'auto';
				p.style.backgroundImage = 'none';
				//tinycontent的内容由函数参数c决定
				b.innerHTML = c
			} else {
				//a=1， u=0 or a=0,u=1
				b.style.display = 'none'; //隐藏tinycontent
				p.style.width = p.style.height = '100px'; //tinybox为正方高宽
			}
			this.mask(); //改变tinymask高宽
		...
		alpha: function (e, d, a, s) {
			clearInterval(e.ai); //tinymask.ai?
			if (d == 1) {
				e.style.opacity = 0;
